...
bean ships a small **core** set, always on. Anything else is a **plugin** (a module you drop into
`~/.bean/plugins/`). Every connector authenticates with the user's own credentials, runs locally, and
lands documents in the DuckDB catalog; the shared chunk → embed → hybrid-search pipeline does the
rest. Where a service has more than one way in, bean implements both and defaults to the path an
individual can self-serve without an admin.
...
Bearer; **Microsoft** mints Graph tokens via device-code or `--method az`.
...
(fake `fetch` → assert `changed`, re-sync no-op, `parse_add` routing). To promote a plugin to core,
move its module into `bean/` and add a row to `CORE_SOURCES` in `bean/sources.py`.
