...
config lives, whether it needs auth, and one `sync()` callable with the same signature — so
`sync.py` and the CLI never special-case a provider.
...
or ship a plugin — copy `docs/connector-template.py` to start."""
...
# resolved `<source>.lookback_days` setting overrides. `bean init` prints the current value per source.
...
CORE_SOURCES: list[Source] = [
    Source("slack", "slack", "Slack", ("channels",), _slack_sync,
    ...
    Source("gdocs", "google", "Google Drive", ("docs", "folders"), _gdocs_sync,
    ...
    Source("jira", "jira", "Jira", ("projects",), jira.sync, auth="jira",
    ...
    Source("zendesk", "zendesk", "Zendesk", ("include",), zendesk.sync,
    ...
    Source("hubspot", "hubspot", "HubSpot", ("include",), hubspot.sync,
    ...
    Source("discord", "discord", "Discord", ("channels", "guilds"), _discord_sync,
...
# Always LAST — the path catch-all. Kept out of CORE_SOURCES so discovered drop-in plugins list
# ahead of the filesystem fallback in `bean init` / `bean status`.
LOCALFILES = Source("localfiles", "localfiles", "Local files", ("paths",), localfiles.sync,
                    auth=None, add_help="a file or folder path")
